 ...
 * @Last Modified time: 2017-06-18 01:14:03
 */
var __alias__ = function(alias){
    __context__.alias[alias.id] = alias.alias
}
var __context__ = function(deps, aliasPath, entryPoint){
    //hashmap
    ...
    var Require = function(from){
        return function(dep){
            var id = path(from, dep)
            var module = __context__.modules[id]
    ...
    function evalCode () {
        for(var id in __context__.entries){
            var module = __context__.modules[id]
            if(!module.excuted){
                var require = new Require(id)
                var exports = module.exports
    ...
    function checkIsAllDepsLoaded () {
        var result = true
        for(var dep in __context__.deps){
    ...
    deps.forEach(function(dep, index){
        ...
        var base =  document.querySelector('[data-main]').getAttribute('data-base') || ''
...
__context__.entries = {};
(function(){
    var els = document.querySelectorAll('[data-main]')
